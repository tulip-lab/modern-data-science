...
1. To use inheritance, we specify the base class name in the `tuple` (more on [`tuple`](M02G-AdvDataTypes.md#tuple)) following the class name in the definition, e.g. **def Teacher(SchoolMember)**. We also need to explicitly call the  **init** method of based class  in the **init** method of subclass. Note that *Python* does not automatically call the constructor of the base class.
